...
#define NUMBEROFSERVERS 7
#define INITIALSERVERBUFFERTASKS 4 //initially 4 tasks added
...
int main(){
    ...
    for(int i=0;i<NUMBEROFSERVERS;i++){
        ...
        serverArray[i]=w;
        for(int j=0;j<INITIALSERVERBUFFERTASKS;j++){
    ...
    while(flag){
        ...
        switch(ch){
            ...
            case 3: //3.Remove task from server i
                cout << "Enter server number to remove task from: (0-" << NUMBEROFSERVERS - 1 << ")" << endl;
                cin>>i;
                if(i>=NUMBEROFSERVERS || i<0)
                    cout<<"Enter a valid choice between 0-"<<NUMBEROFSERVERS-1<<endl;
                else{
